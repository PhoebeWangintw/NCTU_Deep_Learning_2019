...
int main(int argc, const char* argv[]) {
	...
	tdl.load("./model/model_600000");

	// train the model
	std::vector<state> path;
	path.reserve(20000);
	for (size_t n = 600001; n <= total; n++) {
		board b;
		int score = 0;
		...
		while (true) {
			...
			path.push_back(best);  // save record
		...
		// update by TD(0)
        // from terminal downto initial
		tdl.update_episode(path, alpha);  // learn evaluation, modify this
		tdl.make_statistic(n, b, score, 1000);  // unit=100
		path.clear();
        if (n % 10000 == 0)
        {
